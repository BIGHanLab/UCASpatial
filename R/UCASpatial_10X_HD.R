#' @title Ultra-high resolution Cellular annotation of Spatial transcriptomics for 10X HD platform - UCASpatial
#'
#' @description (require Seurat >= 5.1.0) This package could deconvolute the spatial transcriptomics data (10X HD) into cell-type-composition based on a reference scRNA-seq data with appropriate cell-type labels.
#'
...
#' @param spatial.assay optional: set the default spatial.assay for st.vis. By default is Spatial.008um'.
...
UCASpatial_HD_downsample <- function(sc_ref,clust_vr,random.seed,downsample_n,n_cluster,assay,slot)
{
...
meta_filter_HD <- function(sc_ref,meta.assay,meta.resolution,meta.purity,meta.ndims,assay,clust_vr,output_path)
{
  sc_ref_meta <- sc_ref
...
init_nmf_matr_HD <- function (cluster_markers, sc_ref, clust_vr,assay = "RNA", slot = "counts", n_top = NULL)
{
  sc_nmf_ready <-
...
train_nsnmfmod_HD <- function(sc_ref,st_vis_matr,cluster_markers,clust_vr,n_top = NULL,assay = "RNA",slot = "data",nmf.tol=1e-04)
{
  library(RcppML)
...
predict_spatial_mixtures_nmf_weighted_HD <- function (nmf_mod, cluster_markers, mixture_transcriptome, normalize)
{
  ...
  total_t <- round(difftime(Sys.time(), start_t, units = "mins"),2)
  print(sprintf("Time to normalize was %smins", total_t))
  ...
  W <- weight_matr %*% nmf_mod@w[keep_genes,]
  ...
  start_t <- Sys.time()
  coef_pred <- sapply(1:ncol(count_matr), function(i) {
    nnls::nnls(A = W, b = count_matr[, i])$x
  })
  total_t <- round(difftime(Sys.time(), start_t, units = "mins"),2)
  print(sprintf("Time for NNLS was %smins", total_t))
  saveRDS(coef_pred,file = paste(getwd(), '/temp_coef_pred.rds', sep = ""))
  return(coef_pred)
}
