...
#'
#' @description (require Seurat >= 5.1.0) This package could deconvolute the spatial transcriptomics data into cell-type-composition based on a reference scRNA-seq data with appropriate cell-type labels.
#'
...
UCASpatial_downsample <- function(sc_ref,clust_vr,random.seed,downsample_n,n_cluster,assay,slot)
{
  ...
  sc_barcodes <- lapply(names(table(sc_ref@meta.data[[clust_vr]])),function(clus){
    ...
    variances <- lapply(samples,function(x){
      cell_expr <- GetAssayData(object = sc_tmp,assay = assay,layer = slot) %>% as.matrix()
...
{
  sc_nmf_ready <-
    Matrix::Matrix(t(as.matrix(Seurat::GetAssayData(sc_ref, assay = assay,layer = slot))), sparse = T)
...
CalculateEntropy <- function(object,features,assay = "RNA",slot = "data",unit = "log2")
{
  ...
  data.use <- Seurat::GetAssayData(object = object ,  layer = slot,assay = assay) %>% as.matrix()
